...
class Register:
    def __init__(self, root):
        ...
        self.txt_contact = Entry(frame1, font=("times new roman", 15), bg="lightgray", fg="black")
        self.txt_contact.place(x=50, y=200, width=250)

        email = Label(frame1, text="Email ID", font=("times new roman", 15, "bold"), bg="white", fg="black").place(x=370, y=170)
    ...
    def register_data(self):
        if self.txt_fname.get() == "" or self.txt_contact.get() == "" or self.txt_email.get() == "" or self.cmb_quest.get() == "Select" or self.txt_answer.get() == "" or self.txt_password.get() == "" or self.txt_cpassword.get() == "":
            ...
            mb.showerror("Error", "Password & Confirm Password should be the same", parent=self.root,)
        elif self.var_chk.get() == 0:
            mb.showerror("Error", "Please Agree to our terms & conditions", parent=self.root,)
        else:
            try:
                ...
                if row is not None:
                    mb.showerror("Error", "User already Exist, Please try with another email", parent=self.root)
                else:
                    ...
                    con.commit()
                    con.close()
                    mb.showinfo("Success", "Register Successful", parent=self.root)
                    self.clear()
                    self.login_window()
            except Exception as es:
                mb.showerror("Error", f"Error due to: {str(es)}", parent=self.root)
